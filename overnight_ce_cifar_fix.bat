...
for %%S in (%SEEDS%) do (
  ...
  REM si un FICHIER occupe ce chemin (et non un dossier), on le supprime
  if exist "!RUN!" if not exist "!RUN!\NUL" (
    echo [FIX] Found file at !RUN! ; deleting the stray file
    del /f /q "!RUN!"
  )
  ...
  if exist "!RUN!\config.json" if exist "!RUN!\best.pt" (
    ...
    echo [CE][CIFAR-10][seed=%%S] Training to !RUN!
    ...
    if errorlevel 1 (
      echo [ERR] Train errorlevel !ERRORLEVEL! for !RUN! (see !RUN!\train.log)
    )
  )
...
for %%S in (%SEEDS%) do (
  ...
  if exist "!RUN!\config.json" if exist "!RUN!\best.pt" (
    ...
    echo [CE][CIFAR-100][seed=%%S] Training to !RUN!
    call python -m plr_hilbert.train ^
